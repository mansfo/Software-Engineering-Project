...
PGresult *login_customer(Con2DB db, string id){
    char sqlcmd[500];
    snprintf(sqlcmd, 500, "SELECT name,surname,addr FROM Customer WHERE id = '%s'", id.c_str());
    return db.ExecSQLtuples(sqlcmd);
}

int login_with_psw_customer(Con2DB db, Con2DB logdb, string email, string password){
    char sqlcmd[500];
    snprintf(sqlcmd,500,"SELECT id FROM Customer WHERE email = '%s'",email.c_str());
    PGresult *res;
    res = db.ExecSQLtuples(sqlcmd);
    if(!PQntuples(res)) return -1;
    snprintf(sqlcmd,500,"SELECT id FROM Customer WHERE email = '%s' AND password = '%s'",email.c_str(), password.c_str());
    res = db.ExecSQLtuples(sqlcmd);
    if(!PQntuples(res)) {
